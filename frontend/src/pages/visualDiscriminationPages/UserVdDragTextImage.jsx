...
export default function UserVdDragTextImage() {
  ...
  useEffect(() => {
    if (!activity) return;
    const items = activity.items.map((item, i) => ({ ...item, id: `item-${i}` }));
    setDraggableItems(shuffle(items));
    const zones = {};
    activity.targets.forEach((t) => (zones[t] = null));
    setDropZones(zones);
    setResults({});
    setScore(0);
    setFinished(false);
    totalMarks.current = activity.items.reduce((s, it) => s + (it.mark || 2), 0);
  }, [activity]);
  ...
  const handleDropOnZone = (e, target) => {
    ...
    setDraggableItems((cur) => {
      let updated = cur.filter((it) => it.id !== dragging.id);
      if (prev) updated = [...updated, prev];
      return updated;
    });
    const isCorrect = dragging.group === target;
    setScore((s) => {
      let ns = s;
      if (prev && results[prev.id] === "correct") ns -= (prev.mark || 2);
      if (isCorrect) ns += (dragging.mark || 2);
      return ns;
    });
  ...
  const nextActivity = () => {
    if (actIndex < activities.length - 1) setActIndex((i) => i + 1);
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-100 to-indigo-200 flex items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center border-4 border-violet-200">
          ...
          <h2 className="text-3xl font-black text-violet-700 mb-1">
            {percent === 100 ? "නියමයි!" : percent >= 60 ? "හොඳයි!" : "නැවත උත්සාහ කරන්න!"}
          </h2>
          ...
          <div className="space-y-2 mb-5">
            {activity.targets.map((t) => {
              const placed = dropZones[t];
              const correct = placed?.group === t;
              return (
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-indigo-200 select-none p-4 md:p-8">
      ...
      <div className="flex flex-wrap justify-center gap-5 mb-8 max-w-3xl mx-auto">
        {activity.targets.map((target) => {
          const placed = dropZones[target];
          const status = placed ? results[placed.id] : null;
          return (
            <div key={target} className="flex flex-col items-center gap-1.5">
              <div className="w-20 h-20 bg-white border-4 border-violet-300 rounded-2xl flex items-center justify-center text-3xl font-black text-violet-700 shadow">
                {target}
              </div>
              ...
              <div onDrop={(e) => handleDropOnZone(e, target)} onDragOver={handleDragOver}
                className={`w-20 h-20 rounded-2xl border-4 border-dashed flex items-center justify-center transition-all
                  ${placed
                    ? status === "correct" ? "border-green-400 bg-green-50 shadow-lg" : "border-red-400 bg-red-50 shadow-lg"
          ...
          : (
            <div className="flex flex-wrap justify-center gap-3">
              {draggableItems.map((item) => (
                <img key={item.id} src={`${BASE}${item.image_url}`} alt={item.group} draggable
