...
export default function MemoryGamePage() {
  const [screen, setScreen] = useState("level"); // level | image | quiz | result
  ...
  // Load game when level is selected
  ...
  // Countdown timer for image phase
  useEffect(() => {
  ...
  const submitAnswers = () => {
    ...
    game.questions.forEach((q, i) => {
      if (answers[i] !== undefined && parseInt(answers[i]) === parseInt(q.correct)) {
        earned += q.mark || 1;
      }
    });
    setScore(earned);
  ...
  const gradeInfo = percentage >= 80 ? { label: "අති උතුම්! 🌟", color: "#065f46", bg: "#d1fae5" }
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "'Nunito', sans-serif", background: "linear-gradient(135deg, #fdf4ff 0%, #e0f2fe 50%, #fef9c3 100%)" }}>
      <style>{`
        ...
        @keyframes pulse-ring { 0% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.08); opacity: 0.8; } 100% { transform: scale(1); opacity: 1; } }
      ...
      {screen === "level" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: 24 }}>
          <div className="pop-in" style={{ textAlign: "center", marginBottom: 40 }}>
            <div style={{ fontSize: "5rem" }}>🧠</div>
            <h1 style={{ fontWeight: 900, fontSize: "2.4rem", color: "#4f46e5", margin: "12px 0 6px" }}>දෘශ්‍ය ක්‍රීඩාව!</h1>
            <p style={{ color: "#9ca3af", fontWeight: 700, fontSize: "1.1rem" }}>ඔබේ මට්ටම තෝරන්න</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 20, width: "100%", maxWidth: 680 }}>
            {Object.entries(levelConfig).map(([key, cfg], i) => (
              <button
                ...
                style={{
                  background: cfg.gradient, border: `3px solid ${cfg.border}`, borderRadius: 24, padding: "32px 20px",
                  cursor: "pointer", textAlign: "center", transition: "transform 0.2s, box-shadow 0.2s",
                  boxShadow: "0 6px 24px rgba(0,0,0,0.08)", animationDelay: `${i * 0.1}s`,
                }}
      ...
      {screen === "image" && game && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: 24 }}>
          <div className="card pop-in" style={{ width: "100%", maxWidth: 560, padding: "36px 32px", textAlign: "center" }}>
            ...
            <p style={{ color: "#9ca3af", fontWeight: 700, marginBottom: 20 }}>
              ප්‍රශ්න ඇසෙන්න ඉස්සෙල්ලා මේ පින්තූරය මතක තියාගන්න!
            </p>
            ...
            <div style={{ position: "relative", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
              <svg width="90" height="90" viewBox="0 0 90 90" style={{ transform: "rotate(-90deg)" }}>
                ...
                <circle
                  ...
                  stroke={timeLeft <= 5 ? "#ef4444" : "#6366f1"} strokeWidth="7"
                  strokeDasharray="239"
                  strokeDashoffset={239 - (timeLeft / COUNTDOWN) * 239}
                  strokeLinecap="round"
            ...
            <button
              onClick={() => { clearInterval(timerRef.current); setScreen("quiz"); }}
          ...
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            ...
            <div className="card pop-in" style={{ padding: "16px 24px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
              <img
                src={`${BASE}${game.image_url}`}
                alt=""
                style={{ width: 60, height: 50, objectFit: "cover", borderRadius: 12, border: "2px solid #e0e7ff" }}
              />
              <div style={{ flex: 1 }}>
                <p style={{ fontWeight: 900, color: "#1f2937", margin: 0 }}>{game.title}</p>
                <p style={{ color: "#9ca3af", fontWeight: 700, fontSize: "0.8rem", margin: "2px 0 0" }}>
                  {answeredCount}/{game.questions.length} ප්‍රශ්නවලට පිළිතුරු දෙන ලදී
                </p>
              </div>
            ...
            {game.questions.map((q, qIdx) => (
              <div key={qIdx} className="card pop-in" style={{ padding: 24, marginBottom: 20 }}>
                <p style={{ fontWeight: 800, fontSize: "1.05rem", color: "#1f2937", marginBottom: 16 }}>
                  <span style={{ background: "#ede9fe", color: "#6d28d9", borderRadius: 8, padding: "2px 10px", fontSize: "0.8rem", marginRight: 8, fontWeight: 800 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {q.options.map((opt, oIdx) => {
                    ...
                    let btnStyle = {};
                    if (isSelected) {
                      btnStyle = { background: "#ede9fe", borderColor: "#6366f1", color: "#4f46e5" };
                    }
                    return (
                      <button
                        key={oIdx}
                      ...
                      >
                        <span style={{
                          ...
                          fontWeight: 900, flexShrink: 0, fontSize: "0.85rem",
                        }}>
                ...
                <div style={{ marginTop: 10, textAlign: "right" }}>
                  <span style={{ background: "#f3f4f6", color: "#9ca3af", borderRadius: 8, padding: "2px 10px", fontSize: "0.75rem", fontWeight: 700 }}>
                    🏅 {q.mark} ලකුණු
                  </span>
                </div>
              </div>
            ...
            <button
              ...
              style={{
                ...
                background: answeredCount === 0 ? "#e5e7eb" : "linear-gradient(135deg, #6366f1, #ec4899)",
                ...
                boxShadow: answeredCount > 0 ? "0 6px 24px rgba(99,102,241,0.35)" : "none",
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: 24 }}>
          <div className="card pop-in" style={{ maxWidth: 440, width: "100%", padding: "40px 32px", textAlign: "center" }}>
            ...
            <div style={{
              ...
              background: `conic-gradient(#6366f1 ${percentage * 3.6}deg, #e0e7ff 0deg)`,
              display: "flex", alignItems: "center", justifyContent: "center",
              margin: "20px auto", boxShadow: "0 8px 32px rgba(99,102,241,0.25)",
            }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 28 }}>
              <div style={{ background: "#d1fae5", borderRadius: 16, padding: "16px 8px" }}>
                <div style={{ fontWeight: 900, fontSize: "1.6rem", color: "#065f46" }}>
                  {game.questions.filter((q, i) => answers[i] !== undefined && parseInt(answers[i]) === parseInt(q.correct)).length}
                </div>
              ...
              <div style={{ background: "#fee2e2", borderRadius: 16, padding: "16px 8px" }}>
                <div style={{ fontWeight: 900, fontSize: "1.6rem", color: "#991b1b" }}>
                  {game.questions.filter((q, i) => answers[i] !== undefined && parseInt(answers[i]) !== parseInt(q.correct)).length}
                </div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button
                onClick={() => { setScreen("level"); setGame(null); setLevel(null); }}
