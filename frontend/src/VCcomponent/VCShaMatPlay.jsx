...
export default function VCShaMatPlay({ activityId }) {
  ...
  const pick = (opt) => {
    ...
    if (opt.is_correct) {
      ...
      const level = levelMap[(meta.levels || [])[0]];

      axios.post(
        `http://localhost:5000/api/vc_adaptive/complete/shamat/${level}`,
        {},
        {
      ...
      `}</style>
        ...
        {completed && (
          <div className="max-w-lg mx-auto mb-5 bg-green-400 text-white rounded-3xl p-4 text-center shadow-lg pop">
            ...
            <p className="text-green-100 font-semibold mt-1">
              කාලය: {seconds}s • උත්සාහ කිරීම්: {attempts}
            </p>
            <button
              onClick={reset}
        ...
        <div className="flex flex-row justify-center items-start gap-5 flex-wrap">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div
              className="bg-white p-2 rounded-3xl shadow-md border-2 border-indigo-100 flex items-center justify-center"
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            {feedback && !completed && (
              <div
                className={`mt-1 w-full font-extrabold text-base rounded-2xl px-4 py-3 text-center
              ${
                feedback.ok
