...
# -------------------------------
def compute_wrong_ratio(df):
    total_features = df.shape[1]
    low_values = (df <= 0).sum(axis=1)  # assumes wrong answers = 0
    return low_values / total_features

# -------------------------------
# Prediction Route
# Supports old frontend (/predictVDH) and new modular (/predict_vd)
# -------------------------------
@vd_bp.route("/predictVDH", methods=["POST"])   # Keep frontend unchanged
    ...
    try:
        ...
        for i, pred in enumerate(ml_predictions):
            if wrong_ratio.iloc[i] >= 0.8:
                final_predictions.append(0)  # force WEAK
            else:
                final_predictions.append(pred)

        # Get level and advice separately
        ...
        # -------------------------------
        vd_record = VDModel(
            input_marks=data[0] if isinstance(data, list) else data,
